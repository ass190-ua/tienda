    ...
    <v-navigation-drawer :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)"
        ...
        <div class="drawer-inner">
                ...
                <v-fade-transition mode="out-in">
                    ...
                    <div v-else key="items" :class="['items-wrap', { clearing: isClearing }]">
                        <TransitionGroup name="cart" tag="div" class="items-list">
                            <div v-for="it in items" :key="lineKey(it)" class="item-row">
            ...
            <div class="drawer-footer">
                ...
                <v-btn color="primary" variant="flat" rounded="lg" size="large" class="text-none w-100 mb-3"
...
/**
 * Normaliza items por si tu store los guarda como array u objeto.
 * (Si ya es array, perfecto.)
 */
const items = computed(() => {
    const raw = cart.items ?? cart.cartItems ?? []
...
const subtotal = computed(() => {
    ...
    return items.value.reduce((acc, it) => acc + lineTotal(it), 0)
})
...
function itemImage(it) {
    return it.product?.image ?? it.image ?? ''
}

function lineKey(it) {
    const pid = it.product?.id ?? it.id ?? it.productId ?? 'x'
    const s = it.size ?? ''
    const c = it.color ?? ''
    return `${pid}__${s}__${c}`
}

/**
 * Wrappers “tolerantes”:
 * si tu store tiene métodos con otros nombres, intenta encontrarlos;
 * si no, cae a una mutación simple (para que no se rompa).
 */
function inc(it) {
    const key = lineKey(it)
    if (typeof cart.increment === 'function') return cart.increment(key)
    if (typeof cart.increaseQty === 'function') return cart.increaseQty(key)
    if (typeof cart.addToCart === 'function') {
        return cart.addToCart({ product: it.product ?? it, qty: 1, size: it.size ?? null, color: it.color ?? null })
    }
    it.qty = (it.qty ?? 1) + 1
}
...
function dec(it) {
    const key = lineKey(it)
    if (typeof cart.decrement === 'function') return cart.decrement(key)
    if (typeof cart.decreaseQty === 'function') return cart.decreaseQty(key)
    if ((it.qty ?? 1) <= 1) return remove(it)
    it.qty = (it.qty ?? 1) - 1
}

function remove(it) {
    const key = lineKey(it)
    if (typeof cart.removeItem === 'function') return cart.removeItem(key)
    if (typeof cart.removeFromCart === 'function') return cart.removeFromCart(key)
    if (typeof cart.removeLine === 'function') return cart.removeLine(key)

    // fallback si items es array en el store
    if (Array.isArray(cart.items)) {
        cart.items = cart.items.filter((x) => lineKey(x) !== key)
    }
}
